...
## 7. SSOT 산문 ≠ 코드 — 충돌 2종을 어떻게 판단했나

SSOT는 사양의 진실이지만 **표기는 산문**이라 코드 컨벤션과 어긋날 수 있다. Phase 3에서 2건([[Phase_3_Guide]] §0.1):

| | SSOT 산문 | 판단 | 근거 |
|--|-----------|------|------|
| C-1 | 병렬 배열 `skillSlots[]`+`skillCooldowns[]` | `SkillSlot`로 번들 | `feedback_no_parallel_arrays` — 인덱스 동기화 주석은 침묵 버그 |
| C-2 | `IsInPreferredRange(target.position…)` | **거리**로 재해석 | 0~100 축 의미 — 절대 좌표론 게이팅 붕괴 |

**판단 원칙**: SSOT가 정하는 건 *동작*(병렬 배열로 적힌 두 값이 슬롯·쿨이라는 사실, 거리대로 게이팅한다는 사실)이지 *표현*이 아니다. 표현은 프로젝트 컨벤션이 정한다. 그래서:
- C-1은 **동작 보존 + 표현만 컨벤션화** → 가이드가 조용히 컨벤션을 따르고 노트만 남김.
- C-2는 **산문이 모호**(`target.position`이 절대 좌표인지 거리인지)했고, 0~100 축에서 적이 멀리 spawn하고 Player가 진군하는 모델상 "거리"로만 의미가 산다. 이건 단순 표기가 아니라 *의미 해석*이라 가이드에 가장 크게 박아 둔다.
...
2. **심법 중복 skip 분기** — 결정 트리의 *"Focus이고 효과 활성이면 skip"* 은 "효과 활성"을 판정할 효과 시스템(Phase 4)이 있어야 의미. 지금 넣으면 항상 false인 분기.
...
## 11. `Array.Empty<T>()` — 값이 없지만 null은 아닌 배열

`System.Array.Empty<SkillSlotView>()`는 **길이가 0인 배열**을 돌려준다.
의미는 `new SkillSlotView[0]`와 같지만, 매번 새 배열을 만들지 않고 재사용한다.

`ActorView.Skills`처럼 View/Test가 읽는 목록은 `null`보다 빈 배열이 낫다.
이유는 간단하다:

- `null`은 "아직 준비 안 됨"인지 "항목이 없음"인지 헷갈린다.
- 빈 배열은 "목록은 있는데 항목이 0개"라는 뜻이 바로 보인다.
- 받는 쪽에서 `foreach`를 바로 돌릴 수 있어 `if (skills != null)` 같은 방어 코드가 줄어든다.

```csharp
foreach (var skill in actor.Skills)
{
    // 빈 배열이면 본문이 실행되지 않고 끝난다.
}
```

단, 모든 곳에서 빈 배열이 정답은 아니다.
`PlayerStartData.StartingSkills`처럼 입력을 꼭 채워야 하는 자리라면, 값을 만드는 쪽에서 명시적으로 채워서 넘기는 편이 좋다.
빈 배열은 "정상적으로 비어 있음"을 나타낼 때 쓰고, 입력 누락을 숨기는 용도로 쓰지 않는다.

---

## 12. 학습 체크 — Phase 3 마치고 답할 수 있어야 할 것들
