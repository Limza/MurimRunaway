...
- 시드는 `(runSeed, battleIndex)` 의 결합으로 생성. 같은 시드 + 같은 BattleData + 같은 입력 시퀀스 = 같은 결과.
...
{
    void Setup(BattleData data, int seed);
    void Start();
    event Action<BattleSnapshot> OnSnapshot;   // 매 틱 끝
    event Action<BattleResult>   OnResult;     // 종료 시 1회
}
```

**Behaviors**:
1. `TickService` 가 0.05s 간격으로 `OnTick(0.05f)` 호출 (Unity Update 내부에서 누산 후 호출).
...
### Phase 1. Actor & 거리축 + 시간/진군

**Goal**: Player 1명, Enemy 1~4명. 1D 거리축. 시간이 흐르고 적이 진군해 정지 위치까지 옴. HP 정의(아직 데미지 없음).
...
- `Idle` / `Approaching` / `Casting` / `HeavyCharging` / `Stunned` / `Dead`
- 본 Phase에서 사용되는 상태: Idle, Approaching, Dead (남은 상태는 후속 Phase에서 활성화).
...
| maxHp | int | HP | [1, ∞) | 최댓값 |
| position | float | distance unit | [0.0, 100.0] | 0=플레이어 측, 100=원거리 |
...
| maxHp | int | 30 | 최대 HP |
| spawnPosition | float | 100.0 | 스폰 위치 |
| stopPosition | float | 20.0 | Idle로 멈출 위치 |
| approachSpeed | float | 5.0 | 진군 속도 (dist/s) |

`BattleData` (Engine 입력):
| 필드 | 타입 | 설명 |
|------|------|------|
...
| 필드 | 타입 | 설명 |
|------|------|------|
| tickIndex | long | 시작 후 누적 틱 |
| timeSec | float | tickIndex × 0.05 |
...
- Enemy: spawn → Approaching → (position ≤ stopPosition) → Idle
- Player: Idle 고정.
- **Player.position = 0.0 으로 고정** (정식 게임에서도 플레이어는 1D 거리축의 원점). 이동 무공(경공)은 Phase 9에서 별도 처리.

**Behaviors**:
1. `Setup(data, seed)` — Player Actor 1, Enemy Actor[] 생성. Enemy.position = spawnPosition. RngService.Reseed(seed).
2. 매 Tick(dt): 모든 Approaching 적에 대해 `pos -= approachSpeed × dt`. `pos <= stopPosition` 이면 Idle로 전이.
3. 모든 적이 Idle이 되면 BattlePhase = Resolve(victory) 송신 후 종료.
...
- I-1.2: 모든 시점에 `0 ≤ actor.position ≤ 100`.
- I-1.3: 같은 (seed, BattleData) → 모든 BattleSnapshot 시퀀스가 byte-equal (결정성).
...
- Tick dt가 큰 경우(에디터 일시정지 후 재개) `pos`가 stopPosition을 크게 넘어 음수까지 내려갈 수 있음 → `pos = max(pos, stopPosition)` 클램프.

**Acceptance**:
- [ ] Player 1, Enemy 1 (spawn=100, stop=20, speed=5) 설정 → 16.0s ± 0.05s 안에 Idle 전이 + Resolve(victory) 송신.
- [ ] Enemy 4명 동시 진군 시, 모두 결정적으로 같은 시각에 Idle 전이.
- [ ] EditMode: 같은 seed로 동일 BattleData 두 번 실행 → 모든 OnSnapshot 사본이 동일.
- [ ] PlayMode 시각 확인: 플레이어/적 위치가 거리 게이지에 실시간 표시.
...
- `Approach → Engage` 전이: 살아있는 모든 적이 Idle 상태가 된 시점에 BattlePhase = Engage. 이전엔 자동 시전 결정 트리가 동작하지 않는다 (Approach 페이즈 동안 거리 게이팅에 어차피 걸리지만, 페이즈로도 명시적 게이팅).
- `Engage → Approach`: 본 Phase에선 없음 (적이 후퇴하지 않음).
...
- I-3.4: 결정성: 같은 seed/BattleData/SkillSlots → 같은 OnSkillCast 시퀀스.
...
- I-4.3: Dead 상태 액터는 어떤 이벤트도 받지 않는다 (공격·피격·시전·진군 X).
...
- [ ] EditMode: 결정성 — 같은 시드 + 같은 BattleData → OnDamage/OnActorDeath 시퀀스 동일.
...
- `Ihwajeopmok` (이화접목) / `Bobeop` (보법) / `Naryeotagon` (나려타곤) / `Geumgang` (금강불괴) / `Jipae` (지패)
...
| choices | DefenseChoice[] | UI에 노출되는 5개 |
...
2. **방어 선택지 풀 결정**: `choices = availableDefenseChoices` 중 5개 (오성에 따른 풀 차이는 Phase 7에서, 본 Phase에선 5개 고정 노출).
...
| 적 패턴 \ 방어 | 이화접목 | 보법 | 나려타곤 | 금강불괴 | 지패 |
|---------------|---------|------|---------|---------|------|
| 직공 (Direct) | ★ Counter | Normal | Weak | Normal | Normal |
| 연환공 (Combo) | Weak | ★ Counter | Normal | Weak | Normal |
| 허실공 (Feint) | Normal | Weak | Normal | Normal | ★ Counter |
...
- I-5.3: 매트릭스는 9칸 모두 정의되어 있다 (테스트로 검증).
...
**Acceptance**:
- [ ] EditMode: 매트릭스 9칸 모두 기대 결과 반환 (단위 테스트).
...
- [ ] PlayMode: 적 머리 위 ⚠ 차징 표시 + 5택 UI 노출 + 선택 결과 화면 피드백.
...
`OuuiPattern` enum (오의 패턴, 공격 매트릭스의 행 — 플레이어 측 선택):
- `Frontal` (정면강타) / `Penetrate` (관통) / `FeintStrike` (허초후공격)
...
4. **타임아웃 (결정)**: 자동 = `Frontal` (정면강타). 3개 패턴 중 1Counter·1Weak·1Normal 분포로 가장 보수적이며, 0정보 사용자에게 페널티 최소화.
...
|--------------------|-----------------|----------------------|---------------------|
| 정면강타 (Frontal) | Weak | ★ Counter | Normal |
| 관통 (Penetrate) | ★ Counter | Weak | Normal |
| 허초후공격 (FeintStrike) | Normal | Normal | ★ Counter |
...
`DefenseChoicePool` (오성 등급별 노출되는 5택):
- Low: 약한 5개 고정 (예: 보법, 지패, 나려타곤, 흘리기 무용, 무방비) — 카운터 패가 없음
- Mid: 5개 중 카운터 2개 포함
- High: 5개 모두 카운터 가능 (이상적인 풀)
- > 정확한 5개 매핑은 §4 Open Question
...
| Q-6 | 오성 등급별 5택 풀의 정확한 무공 매핑 | P7 | P7 진입 전 |
